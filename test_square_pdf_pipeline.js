...
// 1. Number Cleaning Verification
console.log('1. Testing cleanNumber...');
...
assert.strictEqual(SalesEngine.cleanNumber('9'), 9);
console.log('✓ cleanNumber passed!\n');
...
assert.strictEqual(SalesEngine.normalizeAuditCategory('Gelato & Sorbet Scoops'), 'Ice Cream / Scoops');

// Verify composite category items sum to 5 items and £77.50, and all map to 'Dessert/Waffle'
...
assert.strictEqual(squareCategoryItems.length, 5, `Expected 5 items in Dessert/Waffle group, got ${squareCategoryItems.length}`);
assert.strictEqual(dessertWaffleNet, 77.50, `Expected £77.50 net sales in Dessert/Waffle group, got £${dessertWaffleNet}`);
squareCategoryItems.forEach(item => {
  assert.strictEqual(item.category, 'Dessert/Waffle', `Item ${item.master_item_name} category should be Dessert/Waffle, got ${item.category}`);
});
console.log(`✓ 'Waffles, Pancakes, Cookie Doughs, Cheesecakes' mapped to 'Dessert/Waffle' with 5 items and £${dessertWaffleNet.toFixed(2)} Net Sales`);
